...
function inject (bot, options) {
  ...
  bot._client.on('registry_data', (packet) => {
    ...
    if (codec?.id && Array.isArray(codec.entries)) {
      const safeEntries = codec.entries.flatMap((entry) => {
        if (!entry?.value) return []
        try {
          nbt.simplify(entry.value)
          return [entry]
        } catch {
          return []
        }
      })
      bot.registry.loadDimensionCodec({ ...codec, entries: safeEntries })
      return
    }
